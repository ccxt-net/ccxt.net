...
namespace CCXT.NET.Shared.Coin
{
    ...
    public class ExchangeInfo
    {
        ...
        ///
        /// </summary>
        private AsyncLock __info_async_lock { get; } = new AsyncLock();
        ...
        public void ApiCallLock(ExchangeLimitCalled limitCalled)
        {
            using (__info_async_lock.Lock())
            {
                if (limitCalled != null)
                {
                    ...
                    if (_wait_milli > 0)
                    {
                        Task.Delay((int)_wait_milli).Wait();      //await Task.Delay(_wait_milli);
        ...
        /// <returns></returns>
        public decimal GetAmountMultiplier(string currency_name, decimal value = 1.0m)
        {
            var _result = value;
