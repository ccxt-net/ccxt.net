    ...
    public partial class PrivateApi
    {
        ...
        public async Task Korbit()
        {
            ...
            if (String.IsNullOrEmpty(_api_key.secret_key) == false || XApiClient.TestXUnitMode == XUnitMode.UseJsonFile)
            {
                ...
                if ((_new_address.supported || TestConfig.SupportedCheck) && _new_address.message.IndexOf("assign address not allowed") < 0)
                {
                ...
                if (XApiClient.TestXUnitMode != XUnitMode.UseExchangeServer)
                {
                    ...
                    if ((_withdraw.supported || TestConfig.SupportedCheck) && _withdraw.statusCode != 400)
                    {
                    ...
                    if ((_cancel_withdraw.supported || TestConfig.SupportedCheck) && _cancel_withdraw.message.IndexOf("not_found") < 0)
                    {
