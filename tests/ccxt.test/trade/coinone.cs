    ...
    public partial class TradeApi
    {
        ...
        public async Task Coinone()
        {
            ...
            if (String.IsNullOrEmpty(_api_key.secret_key) == false || XApiClient.TestXUnitMode == XUnitMode.UseJsonFile)
            {
                ...
                if (_fetch_my_orders.supported || TestConfig.SupportedCheck)
                {
                ...
                if ((_fetch_my_order.supported || TestConfig.SupportedCheck) && _fetch_my_order.statusCode != 104)
                {
                ...
                if (_fetch_open_orders.supported || TestConfig.SupportedCheck)
                {
                ...
                if (XApiClient.TestXUnitMode != XUnitMode.UseExchangeServer)
                {
                    ...
                    if ((_limit_order.supported || TestConfig.SupportedCheck) && _limit_order.statusCode != 103)
                    {
                    ...
                    if ((_market_order.supported || TestConfig.SupportedCheck) && _market_order.statusCode != 103)
                    {
                ...
                var _cancel_order = await _trade_api.CancelOrderAsync("XRP", "KRW", _order_id, 0.1m, 20000000m, SideType.Ask, GetJsonContent(_trade_api.tradeClient, "cancelOrder", _args));
                if ((_cancel_order.supported || TestConfig.SupportedCheck) && _cancel_order.statusCode != 104)
                {
                ...
                var _cancel_orders = await _trade_api.CancelOrdersAsync("ETH", "XRP", _oder_ids, GetJsonContent(_trade_api.tradeClient, "cancelOrders", _args));
