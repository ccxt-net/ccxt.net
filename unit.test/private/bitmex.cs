...
namespace XUnit
{
    public partial class PrivateApi
    {
        ...
        public async void BitMEX()
        {
            ...
            {
                var _private_api = new CCXT.NET.BitMEX.Private.PrivateApi(_api_key.connect_key, _api_key.secret_key);
                ...
                var _new_address = await _private_api.CreateAddress("BTC", GetJsonContent(_private_api.privateClient, "createAddress", _args));
                ...
                var _address = await _private_api.FetchAddress("BTC", GetJsonContent(_private_api.privateClient, "fetchAddress", _args));
                ...
                if (XApiClient.TestXUnitMode != XUnitMode.UseExchangeServer)
                {
                    var _withdraw = await _private_api.CoinWithdraw("BTC", "3DE2DBYHC6dj2DZSEDamwnXffzyNHuAr6v", "", 0.1m, GetJsonContent(_private_api.privateClient, "coinWithdraw", _args));
                    if ((_withdraw.supported == true || TestConfig.SupportedCheck == true) && _withdraw.message.IndexOf("amount is invalid") < 0)
                    {
                    ...
                    var _cancel_withdraw = await _private_api.CancelCoinWithdraw("BTC", _withdraw.result.transferId, GetJsonContent(_private_api.privateClient, "cancelCoinWithdraw", _args));
...
#endif
                var _fetch_transfers = await _private_api.FetchTransfers("BTC", _timeframe, _since, _limit, GetJsonContent(_private_api.privateClient, "fetchTransfers", _args));
                if ((_fetch_transfers.supported == true || TestConfig.SupportedCheck == true) && _fetch_transfers.errorCode != ErrorCode.OrderNotFound)
                {
                ...
                var _fetch_transfer = await _private_api.FetchTransfer("BTC", _transfer_id, GetJsonContent(_private_api.privateClient, "fetchTransfer", _args));
                ...
                var _balance = await _private_api.FetchBalance("BTC", "USD", GetJsonContent(_private_api.privateClient, "fetchBalance", _args));
                ...
                var _wallet = await _private_api.FetchWallet("BTC", "USD", GetJsonContent(_private_api.privateClient, "fetchWallet", _args));
