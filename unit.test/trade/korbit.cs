...
namespace XUnit
{
    public partial class TradeApi
    {
        ...
        public async void Korbit()
        {
            ...
            {
                var _trade_api = new CCXT.NET.Korbit.Trade.TradeApi(_api_key.connect_key, _api_key.secret_key, _api_key.user_name, _api_key.password);
                ...
                if (_fetch_my_orders.result.Count <= 0)
                    _my_order_id = _trade_api.tradeClient.GenerateNonceString(16, 8);
                else
                ...
                var _cancel_order = await _trade_api.CancelOrder("XRP", "KRW", _order_id, 1.0m, 20000m, SideType.Ask, GetJsonContent(_trade_api.tradeClient, "cancelOrder", _args));
                ...
                var _cancel_orders = await _trade_api.CancelOrders("XRP", "KRW", _oder_ids, GetJsonContent(_trade_api.tradeClient, "cancelOrders", _args));
